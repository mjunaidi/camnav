...
/* Directives */
var articleModule = angular.module('myApp.article', ['ngRoute', 'ngResource']);

articleModule.run(['ArticleService', function(articleService) {
  articleService.init();
}]);

articleModule.factory('ArticleService', ['$rootScope', '$http', 'Storage', function($rootScope, $http, Storage) {
  var service = {};
  
  service.articles = null;
  service.article = null;
  ...
  service.nav = '/article';
  service.path = 'article/api/';
  
  service.init = function() {
    service.initRowCount();
  };
  
  service.initArticlePage = function(route) {
    var params = route.current.params;
    if (params.id) {
      service.initArticle(params.id);
    } else {
      service.initNewArticle();
    }
  };
  
  service.viewArticles = function(location) {
    location.path(service.nav);
  };
  
  service.addNewArticle = function(location, website) {
    service.website = website;
    location.path(service.nav + '/add');
  };
  
  service.editArticle = function(location, id) {
  ...
  service.initNewArticle = function() {
    service.article = {};
  };
  
  service.initArticle = function(id) {
    var url = service.path + 'get/' + id;
    $http.get(url).
      success(function(data, status, headers, config) {
        service.article = data;
  ...
  service.removeArticle = function(id) {
    if (id) {
  ...
  service.removeAllArticles = function() {
  ...
  service.setArticles = function(articles) {
  ...
  service.updateArticle = function(article) {
    ...
    if (service.website) {
      article.website = service.website;
    }
    
    if (!article.id) {
      url = service.path + 'add';
    } else {
      url = service.path + 'update/' + article.id;
    }
  ...
  service.searchArticles = function(keyword) {
    var first = 0;
